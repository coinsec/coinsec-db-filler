...
class CoinsecdMultiClient(object):
    ...
    async def request(self, command, params=None, timeout=60):
        try:
            return await self.__get_coinsecd().request(command, params, timeout=timeout, retry=1)
        except CoinsecdCommunicationError:
            await self.initialize_all()
            return await self.__get_coinsecd().request(command, params, timeout=timeout, retry=3)
